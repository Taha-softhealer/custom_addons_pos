        ...
        if (this.config.sh_lot_expiry_warning || this.config.sh_restrict_lot_expiry && (product.alert_time || product.use_expiration_date) ) {
            ...
            if (
                ...
                this.dialog.add(ConfirmationDialog, {
                    ...
                    body: "The " + lotName +" has been expired on " + expiry_date?.toLocaleDateString() + " so you can't sell it ",
                });
                return;
            } else if (-1 < daysDifference && Math.round(daysDifference) <= daysToAdd && expiry_value && this.config.sh_lot_expiry_warning) {
                const _confirmed = await ask(this.dialog, {
                    title: "Expiration Alert",
                    body: "The "+ lotName +" will expire on " + expiry_date?.toLocaleDateString(),
                    confirmLabel: "Okay",
                });
                if (!_confirmed) {
                    return
